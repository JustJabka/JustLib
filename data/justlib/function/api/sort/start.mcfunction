...
# 2. in:{target:"element", scale:100, mode:"desc", data:[0.0,3.14,1.5,2.25]}

execute unless data storage justlib:sort in.scale run data modify storage justlib:sort in.scale set value 1
execute unless data storage justlib:sort in.mode run data modify storage justlib:sort in.mode set value "asc"
